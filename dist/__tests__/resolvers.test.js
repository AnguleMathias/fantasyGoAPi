...
import { validateMobileNumber, validatePassword, } from "../src/utils/validators";
...
describe("GraphQL Resolvers", () => {
    describe("getToken Resolver", () => {
        ...
        it("should generate a token for valid credentials", () => {
            validateMobileNumber.mockReturnValue(true);
            validatePassword.mockReturnValue(true);
            generateToken.mockReturnValue("mocked_token");
            const result = getToken({}, { mobileNumber: "0123456789", password: "Password1" });
        ...
        it("should throw an error for invalid credentials", () => {
            validateMobileNumber.mockReturnValue(false);
    ...
    describe("getPlayers Resolver", () => {
        ...
        it("should fetch players for a valid token and league", async () => {
            ...
            fetchPlayers.mockResolvedValue(["player1", "player2"]);
            const result = await getPlayers({}, { league: "EPL" }, { token: "valid_token" });
        ...
        it("should throw an AuthenticationError for an invalid token", async () => {
            ...
            await expect(getPlayers({}, { league: "EPL" }, { token: "invalid_token" })).rejects.toThrow(AuthenticationError);
        });
    });
});
